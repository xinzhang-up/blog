...
<span class="line"><span>                &#39;}&#39;;</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre></div><h4 id="测试" tabindex="-1">测试 <a class="header-anchor" href="#测试" aria-label="Permalink to &quot;测试&quot;">​</a></h4><p>至此， 基础的配置都已完成，后续我们再来针对一个个功能点做分析， 先做简单的消息测试，配置完成后结构如下 <img src="`+l+`" alt="1741671447582"> 启动消费者服务, 发送TestMessage1和TestMessage2, 两个消费者能够正常接收到消息并消费</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>@Test</span></span>
...
<span class="line"><span>                .with(MqConstants.QUEUE_TEST3_DELAY_BINDING_KEY)</span></span>
<span class="line"><span>                .noargs();</span></span>
<span class="line"><span>    }</span></span></code></pre></div><p>从管理界面上也能看到各个标签, 鼠标挪到标签上会有信息显示 <img src="`+i+`" alt="1741671806956"></p><h4 id="利用死信交换机插件实现" tabindex="-1">利用死信交换机插件实现 <a class="header-anchor" href="#利用死信交换机插件实现" aria-label="Permalink to &quot;利用死信交换机插件实现&quot;">​</a></h4><p>这一实现更加简单, 安装并启用插件后, 声明交换机的delay属性 一个消息变成死信的条件有： 1.消费被拒绝（basic.reject 或者 basic.nack），并且参数 requeue = false 时 2.消息TTL（存活时间）过期 3.队列达到最大长度 rabbitMQ对于死信消息的处理是：如果配置了死信队列，成为死信的消息会被丢进死信队列，如果没有则被丢弃。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>/**</span></span>
<span class="line"><span>     * 示例交换机</span></span>
<span class="line"><span>     */</span></span>
<span class="line"><span>    @Bean</span></span>
<span class="line"><span>    public Exchange demoExchange() {</span></span>
<span class="line"><span>        return ExchangeBuilder.topicExchange(MqConstants.EXCHANGE_DEMO).delayed().build();</span></span>
<span class="line"><span>    }</span></span></code></pre></div><p>管理界面也能看到交换机类型变为&quot;x-delayed-message&quot; <img src="`+t+`" alt="1741671841682"> 给消息设置一定的延迟时间, 这里也要注意是消息的&quot;x-delay&quot;属性 交换机会等待delay的时间, 然后再将消息转发到对应的队列中</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>rabbitTemplate.convertAndSend(message.exchange(), message.routingKey(), message, msg -&gt; {</span></span>
<span class="line"><span>                if (message.getPriority() != null) {</span></span>
<span class="line"><span>                    msg.getMessageProperties().setPriority(message.getPriority());</span></span>
<span class="line"><span>                }</span></span>
<span class="line"><span>                if (message.getDelayMinutes() != null) {</span></span>
...
<span class="line"><span></span></span>
<span class="line"><span>}</span></span></code></pre></div><h4 id="监听容器开关" tabindex="-1">监听容器开关 <a class="header-anchor" href="#监听容器开关" aria-label="Permalink to &quot;监听容器开关&quot;">​</a></h4><p>根据业务区分不同的监听容器, 个业务组下的交换机注册到各监听容器中, 通过http请求方式直接关闭开启监听容器</p>`,99)]))}const q=n(c,[["render",o]]);export{h as __pageData,q as default};
